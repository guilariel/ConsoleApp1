...
namespace ActualizeDataBaseWithRabbitMQ.Infrastructure.SellStocksCruds
{
    public class SellStocksCrudUsers
    {
        private readonly SellStocksDbContext _context;

        public SellStocksCrudUsers(SellStocksDbContext context)
        {
            _context = context;
        }
        ...
        {
            UsersDb? userDb = await GetOneByNameAsync(user.name);
            if (userDb == null)
            {
            ...
            {
                await SellFundsAsync(userDb.id, user.funds);
            }
        }
